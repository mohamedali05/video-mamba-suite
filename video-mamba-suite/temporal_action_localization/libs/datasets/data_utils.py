...
def truncate_feats(
    ...
    if feat_len <= max_seq_len:
        ...
        # randomly crop the seq by setting max_seq_len to a value in [l, r]
            ...
            # définit max_seq_len comme étant compris entre 0.9* feat_len et 1*feat_len
    ...
    for _ in range(max_num_trials):
        ...
        #print(f'window shape  : {window.shape} window {window}')

        left = torch.maximum(window[:, 0], data_dict['segments'][:, 0])
        #print('Left segments :')
        #print(data_dict['segments'][:, 0])
        #print(f'left : {left}')

        #print('Right segments :')
        #print(data_dict['segments'][:, 1])
        ...
        #print(f'inter_ratio : {inter_ratio}')
        ...
        if no_trunc:
            ...
            if seg_idx.sum().item() > 0:
                #print(f' segment somme : {seg_idx.sum().item()}')
                break
        else:
    ...
    new_shape =  data_dict['feats'].shape[1]

    #print(f'original feat_len : {feat_len} ; after processing {new_shape}')
