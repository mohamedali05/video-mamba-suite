...
################################################################################
class AverageMeter:
    """A simple class to keep track of average values."""
...
def valid_one_epoch(
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            ...
            # print(output) #Save in a JSON file Containing all predictions
            # check segments unit
    ...
    print("Results before post_process : ")
    str_res_shape= results['t-start'].shape
    res_start_type = type(results['t-start'])
    print(f'results shape {str_res_shape} results_type {res_start_type}')
    ...
    if evaluator is not None:
        if (ext_score_file is not None) and isinstance(ext_score_file, str):
            #the post_process is not executed with the tennis dataset
            results = postprocess_results(results, ext_score_file)
    # call the evaluator



    ap, mAP , Total_mean_AP, f1 , mF1, total_mean_f1  = evaluator.evaluate(results, verbose=True , file_path = file_path , best_file_path = best_file_path  , best_MAP = best_mAP , curr_epoch = curr_epoch)
    ...
    return  ap, mAP , Total_mean_AP, f1 , mF1, total_mean_f1
